...
export default function Dashboard() {
  ...
  const [stats, setStats] = useState({ totalFiles: 0, totalStorage: 0 });
  ...
  const filteredFiles = sortedFiles.filter(file => {
    ...
    const matchesFilter = activeFilter === 'all' || (activeFilter === 'image' && file.type?.startsWith('image/')) || (activeFilter === 'pdf' && file.type === 'application/pdf') || (activeFilter === 'document' && (file.type?.includes('text') || file.type?.includes('doc') || file.type?.includes('xls'))) || (activeFilter === 'other' && !file.type?.startsWith('image/') && file.type !== 'application/pdf' && !file.type?.includes('text') && !file.type?.includes('doc') && !file.type?.includes('xls'));
  ...
  const handleBulkDownload = async () => { // Standard approach: trigger individual downloads
    ...
    addToast(`Starting download of ${selectedFileIds.size
      } files`);
  };
  ...
  return (<div className="min-h-screen bg-zinc-950 text-zinc-100 selection:bg-blue-500/30 overflow-hidden relative"
    ...
    <DotGrid dotSize={2}
      ...
      className="fixed inset-0 pointer-events-none opacity-50" /> {/* 🌪️ Global Drag Overlay */}
    {isDraggingOver && (
      <div
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            <button
              ...
              className="w-10 h-10 flex items-center justify-center rounded-xl bg-linear-to from-blue-600 to-indigo-700 text-white font-bold text-sm shadow-lg shadow-blue-500/25 hover:scale-105 active:scale-95 transition-all cursor-pointer overflow-hidden"
            >
        ...
        <div className="glass p-6 rounded-3xl border border-white/10 group hover:border-blue-500/30 transition-all duration-500">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">{statsLoading ? "..." : stats.totalFiles}</p>
            </div>
          </div>
          <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
            <div className="h-full bg-blue-500 rounded-full" style={{ width: `${Math.min((stats.totalFiles / 50) * 100, 100)}%` }} />
          </div>
        </div>
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">
                {statsLoading ? "..." : `${(stats.totalStorage / (1024 * 1024)).toFixed(1)} MB`}
              </p>
            </div>
          </div>
          <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${Math.min((stats.totalStorage / (500 * 1024 * 1024)) * 100, 100)}%` }}
            />
          </div>
        </div>
